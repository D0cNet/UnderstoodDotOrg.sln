...
function getFilterTerm(filterString) {
    if (typeof filterString === "string") {
        var terms = filterString.split("|");
        var idx = terms.length < 2 ? 0 : 1;
        return terms[idx];
    }
    return "";
}
    ...
    $j.each(resultCallBack.items,
        function () {
            if (this.Name != null) {
                ...
                var template = '<li id="' + this.ItemId + '" class="BlogPostArea" onclick="{0}" style="margin-left:' + InnerItem(this) + '"><div class="BlogPostViews">' + '<a class="ceebox imgcontainer"  title="" href="#"  onclick="{0}"><img width="' + imageWidth + '" onerror="this.onerror=null;this.src=\'../Buckets/images/default.jpg\';" height="' + imageHeight + '" src="' + this.ImagePath + '?w=' + imageWidth + '&h=' + imageHeight + '&db=master" ' + resizeTemplateIcon + '  class="attachment-post-thumbnail wp-post-image" alt="' + this.Name + '" title="' + this.Name + ' - ' + this.Path + '" /></a></div><h5 class="BlogPostHeader"><a href="javascript:void(0);" onclick="scForm.browser.clearEvent(event || window.event, true);{0}">' + this.Name + '</a><span title="This item has ' + (this.Languages.length > 1 ? "" + this.Languages.length + " languages" : " 1 language") + '">' + (this.Languages.length > 1 ? "(" + this.Languages.length + ")" : "") + '</span></h5><div class="BlogPostContent"><strong>' + templateStub + ': </strong>' + this.TemplateName + ' <strong>' + locationStub + ': </strong>' + this.Bucket + '</div><div class="BlogPostFooter">' + (this.Content.length > 250 ? (this.Content.substring(0, 250) + "...") : this.Content) + ' <div><strong>' + versionStub + ': </strong>' + this.Version + ' <strong>' + createdStub + ': </strong>' + this.CreatedDate.substring(0, 10) + ' <strong> ' + byStub + ': </strong> ' + this.CreatedBy + ' <strong> ' + languageStub + ': </strong> ' + this.Language + ' </div><div></div><div class="quickactions" onclick="scForm.browser.clearEvent(event || window.event, true);">' + actionList + mediaCommand + languageList + '</div></li>';
...
$j(function () {
    ...
    $j(".SearchOperation").live("click", function () {
        if ((!$j('.addition').val().replace(/\s/g, '').length && $j('.boxme').children('li').length <= 1)) {
                            ...
                            if (this.className.indexOf("id") > -1) {
                                ...
                                if (listOfSearches.length == 1) {
                                    ...
                                    for (var i = 0; i < listOfSearches.length; i = i + 2) {
                                        var searchText = scHtmlEscape(listOfSearches[i + 1]);
                                        if (searchText != "") {
                                            var split;
                                            if ($j.browser.msie) {
                                            ...
                                            if (childCheck.text().indexOf(split[1].replace(';', '')) < 0) {
                                                $j(".boxme").prepend('<li class="token-input-token-facebook" title="' + searchText + '"><span title="' + changeSearchLogic + '" style="background: url(\'images/' + listOfSearches[i] + '.gif\') no-repeat center center;padding: 0px 11px;"></span><span style="text-overflow: ellipsis;color:black;max-width: 411px;overflow: hidden;vertical-align:top;" class="' + listOfSearches[i] + '">' + searchText + '</span><span class="token-input-delete-token-facebook remove" title="' + removeSearchTerm + '">×</span></li>');
                                                scFixIeLayout();
                                            }
                                        }
                                    }
    ...
    $j(".boxme").on("click", ".token-input-token-facebook .booleanOperation", function () {
        ...
        switch (booleanOperationClass) {
            case 'should':
                ...
                $j(this).css('background-image', 'url(' + getFilterImageIconPath(type, b) + ')');
                break;
            case 'not':
            ...
            case 'booleanOperation':
                ...
                $j(this).css('background-image', 'url(' + getFilterImageIconPath($j(this).next().next().attr('class').split(" ")[0], "must") + ')');
                break;
        }
    ...
    function addFilter() {
        var searchQuery = GetSearchQuery(window.filterForSearch);
        ...
        if (searchQuery.length > 0) {
            ...
            $j.each(searchQuery, function () {
                var d = scHtmlEscape(this.value);
                var friendlyName = scHtmlEscape(this.friendlyName);
                ...
                if (this.type == "sort") {
                    ...
                    a.find(".boxme").prepend('<li class="token-input-token-facebook" style="opacity:0.3;" title="' + d + '"><span  title="' + '' + '" style="background: url(\'images/' + this.type + this.operation + '.png\') no-repeat center center;padding: 0px 11px;" class="booleanOperation ' + this.operation + '"></span>' + dateDescription + '<span class="' + this.type + hidden + ieMaxWidth + '" style="text-overflow: ellipsis;color:black;max-width: 411px;overflow: hidden;vertical-align:top;">' + friendlyName + '</span><span class="token-input-delete-token-facebook remove" title="' + '' + '">×</span></li>');
                }
                scFixIeLayout();
            });
