...
U.popovers = function() {
	...
	self.mobilePositionFix = function() {
		...
		popoverElement.css({
			'left': (leftPos + popoverAdjustLeft) + 'px'
		});
	};
	...
	self.rightSideFix = function() {
		...
		if (bodyPopoverDifference < popoverOffsetLeft) {
			...
			popoverElement.css({
				'left': (popoverPositionLeft - arrowAdjustLeft) + 'px'
			});
		}
	};
...
function DivideDetailPages() {
	...
	$('.about-experts-listing').find('.about-expert-row').remove();
